...
const options:FilterType[]=['Today','Week','Month']
  ...
  return (
    <View className='flex-row self-center w-full justify-between bg-white p-1 rounded-full border border-gray-200'>
      {
        options.map((option)=>(
          <TouchableOpacity
             key={option}
             onPress={() => onChange(option)}
             className={`px-[40px] py-[12px] rounded-full ${activeFilter===option?'bg-primary':''} `}
          >
